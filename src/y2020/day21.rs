...
fn solve_both_parts(input: &str) -> Result<(usize, String), SimpleError> {
    ...
    let num_allergens = food.iter()
        .flat_map(|food_item| food_item.allergens.iter().copied())
        .collect::<HashSet<_>>()
        .len();

    let mut unknown_ingredients: HashSet<_> = food.iter()
    ...
    while allergen_to_ingredient.len() < num_allergens {
        ...
        for food_item in &food {
            let possible_ingredients: Vec<_> = food_item.ingredients.iter().copied()
                .filter(|&ingredient| unknown_ingredients.contains(ingredient))
                .collect();

            for &allergen in &food_item.allergens {
                if let Some(existing_ingredients) = allergen_to_possible_ingredients.get_mut(allergen) {
                    *existing_ingredients = existing_ingredients.iter().copied()
        ...
        let found_ingredients: Vec<_> = allergen_to_possible_ingredients.into_iter()
    ...
    let allergen_free_ingredient_count = food.iter()
        .map(|food_item| {
            food_item.ingredients.iter()
                .filter(|&&ingredient| unknown_ingredients.contains(ingredient))
                .count()
        })
        .sum();
    ...
    let dangerous_ingredient_list: Vec<_> = dangerous_ingredients.into_iter()
        .map(|(_, ingredient)| ingredient)
        .collect();

    Ok((allergen_free_ingredient_count, dangerous_ingredient_list.join(",")))
}
...
fn parse_input(input: &str) -> Result<Vec<FoodItem>, SimpleError> {
    input.lines().map(|line| {
        ...
        Ok(FoodItem { ingredients, allergens })
    })
        .collect()
}
...
mod tests {
    ...
    fn test_sample_input() {
        assert_eq!(Ok((5, String::from("mxmxvkd,sqjhc,fvjkl"))), solve_both_parts(SAMPLE_INPUT));
    }
}