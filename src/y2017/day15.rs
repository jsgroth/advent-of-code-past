...
fn solve_part(input: &str, wait_for_multiples: bool, rounds: usize) -> Result<usize, SimpleError> {
    ...
    if start_values.len() != 2 {
        return Err(SimpleError::new(format!("input has {} lines, expected 2", start_values.len())));
    }
...
mod tests {
    ...
    fn test_sample_input_part_1() {
        assert_eq!(Ok(588), solve_part("Generator A starts with 65\nGenerator B starts with 8921", false, 40_000_000));
    }

    #[test]
    fn test_sample_input_part_2() {
        assert_eq!(Ok(309), solve_part("Generator A starts with 65\nGenerator B starts with 8921", true, 5_000_000));
    }
}